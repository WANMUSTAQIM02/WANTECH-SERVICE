...
import { getFirestore, collection, addDoc, deleteDoc, onSnapshot, query, orderBy, updateDoc, doc, where, limit, getDoc, getDocs, setDoc } from "https://www.gstatic.com/firebasejs/12.15.0/firebase-firestore.js";
...
// FUNGSI BARU: Hantar Dokumen (PDF) ke Telegram
async function sendTelegramDocument(chatId, blob, filename, caption = "") {
    ...
    } catch (err) { console.error("Telegram Document Error:", err); }
}
...
// ==========================================
// STATIC PRICING ENGINE (HARGA TETAP)
// ==========================================
let availableServices = {}; 
let currentCurrency = "MYR"; 
let EXCHANGE_RATE = 4.50; 
...
function renderServerStatus(dataArray) {
    ...
    if (uptimePercentage >= 90) {
        bannerIcon.className = "banner-icon online"; bannerIcon.innerHTML = "✓";
        bannerIcon.style.backgroundColor = "rgba(40, 167, 69, 0.1)"; bannerIcon.style.color = "#28a745"; bannerIcon.style.border = "none";
        bannerTitle.innerText = "All Systems Operational"; uptimeText.style.color = "#28a745"; uptimeFill.style.backgroundColor = "#28a745";
    } else if (uptimePercentage >= 40) {
        bannerIcon.className = "banner-icon warning"; bannerIcon.innerHTML = "⚠️"; 
        bannerIcon.style.backgroundColor = "rgba(255, 149, 0, 0.1)"; bannerIcon.style.color = "#FF9500"; bannerIcon.style.border = "1px solid rgba(255, 149, 0, 0.3)";
        bannerTitle.innerText = "Partial Service Disruption"; uptimeText.style.color = "#FF9500"; uptimeFill.style.backgroundColor = "#FF9500";
    } else {
        bannerIcon.className = "banner-icon offline"; bannerIcon.innerHTML = "✕";
        bannerIcon.style.backgroundColor = "rgba(255, 59, 48, 0.1)"; bannerIcon.style.color = "#FF3B30"; bannerIcon.style.border = "1px solid rgba(255, 59, 48, 0.3)";
        bannerTitle.innerText = "Major Service Disruption"; uptimeText.style.color = "#FF3B30"; uptimeFill.style.backgroundColor = "#FF3B30";
    }
}
...
async function fetchLivePhoenixStatus() {
    ...
    try {
        ...
        if(proxyUrl.includes("MASUKKAN_LINK")) throw new Error("SILA MASUKKAN LINK GOOGLE SCRIPT PHOENIX");
        ...
        let servicesArray = [];
        if (rawData && Array.isArray(rawData.services)) servicesArray = rawData.services; 
        else if (Array.isArray(rawData)) servicesArray = rawData; 

        if (servicesArray.length > 0) renderServerStatus(servicesArray); 
        else throw new Error("Data ditarik tapi senarai kosong.");
    } catch (error) { 
        console.error("Sistem Live terputus:", error.message);
        const grid = document.getElementById('server-status-grid');
        if(grid) grid.innerHTML = `<div style="grid-column: 1 / -1; text-align: center; padding: 40px; background: rgba(255, 59, 48, 0.1); border: 1px dashed #FF3B30; border-radius: 12px; color: #FF3B30;"><h3 style="margin-top: 0;">⚠️ Server Connection Lost</h3><p style="font-size: 0.9rem;">Error: ${error.message}</p></div>`;
        ...
        const uptimeText = document.getElementById('uptime-percent-text'); const uptimeFill = document.getElementById('uptime-bar-fill');
...
function updateLiveClock() {
    ...
    if(!timeEl) return;
    setInterval(() => { timeEl.innerText = new Date().toLocaleTimeString('en-US', { hour12: false }); }, 1000);
}
...
// ==========================================
// AUTO-INVOICE PDF GENERATOR ENGIN (jsPDF)
// ==========================================
// Ditambah parameter 'returnBlob' untuk membolehkan fail dihantar ke Telegram
window.generateInvoice = function(bookingData, returnBlob = false) {
    if (!window.jspdf) {
        console.error("Pustaka jsPDF tidak dijumpai di index.html!");
        return null;
    }
    ...
    doc.text("WAN TECH SOFTWARE", 105, 25, { align: 'center' });
    
    doc.setFont("Helvetica", "normal"); doc.setFontSize(10);
    doc.setTextColor(120, 120, 120);
    doc.text("Premium Mobile Software Maintenance & Servicing", 105, 32, { align: 'center' });
    
    doc.setFont("Helvetica", "bold"); doc.setFontSize(12);
    doc.setTextColor(30, 30, 30);
    doc.text("RESIT TEMPAHAN / INVOICE RASMI", 105, 40, { align: 'center' });
    ...
    printRow("Tracking ID", bookingData.trackingId);
    printRow("Customer Name", bookingData.customerName);
    printRow("Phone Number", bookingData.phoneNumber);
    printRow("Device Model", bookingData.deviceModel);
    printRow("Service Type", bookingData.serviceType);
    if(bookingData.imei) printRow("IMEI / Serial No.", bookingData.imei);
    printRow("Tarikh Tempahan", new Date().toLocaleString());
    ...
    doc.text("Anggaran Harga (Estimated Price)", 22, y + 11);
    
    const finalFormattedPrice = formatPrice(bookingData.estimatedPrice);
    doc.text(finalFormattedPrice, 185, y + 11, { align: 'right' });

    doc.setFont("Helvetica", "normal"); doc.setFontSize(9); doc.setTextColor(140, 140, 140);
    doc.text("Sila simpan salinan PDF ini atau tangkap layar (screenshot) sebagai bukti tempahan rasmi anda.", 105, y + 35, { align: 'center' });
    doc.text("Terima kasih kerana memilih WAN TECH SOFTWARE.", 105, y + 41, { align: 'center' });

    if (returnBlob) {
        return doc.output('blob'); // Pulangkan fail mentah untuk Telegram
    } else {
        doc.save(`WAN_TECH_INVOICE_${bookingData.trackingId}.pdf`); // Download ke phone/PC customer
    }
};
...
document.getElementById('booking-form')?.addEventListener('submit', async (e) => {
    ...
    const bookingData = {
        trackingId: trackingId, customerName: document.getElementById('cust-name').value,
        phoneNumber: document.getElementById('cust-phone').value, deviceModel: document.getElementById('device-model').value,
        serviceType: sName, imei: document.getElementById('imei').value, description: document.getElementById('description').value,
        telegramId: "", estimatedPrice: sPrice, status: "Pending Approval", adminRemark: "", createdAt: new Date(), updatedAt: new Date()
    };

    try {
        await addDoc(collection(db, "bookings"), bookingData);
        
        // SUNTIKAN 1: Auto-Download Invoice PDF di browser customer
        window.generateInvoice(bookingData);
        
        // SUNTIKAN 2: Jana fail mentah PDF untuk dihantar ke Telegram Admin
        const pdfBlob = window.generateInvoice(bookingData, true);

        alert(`Booking successful! Your Invoice PDF has been downloaded automatically.\nYour Tracking ID is: ${trackingId}\nSave this ID and navigate to the 'Track' tab to connect your Telegram.`);
        ...
        try {
            const adminChatsSnap = await getDocs(collection(db, "admin_telegrams"));
            if (!adminChatsSnap.empty) {
                adminChatsSnap.forEach(docSnap => {
                    sendTelegramMessage(docSnap.id, adminMsg);
                    sendTelegramDocument(docSnap.id, pdfBlob, `INVOICE_${trackingId}.pdf`); // Hantar PDF ke Telegram bos
                });
            } else {
                sendTelegramMessage(ADMIN_CHAT_ID, adminMsg); 
                sendTelegramDocument(ADMIN_CHAT_ID, pdfBlob, `INVOICE_${trackingId}.pdf`); 
            }
        } catch(err) {
            sendTelegramMessage(ADMIN_CHAT_ID, adminMsg);
        }
...
// ==========================================
// TELEGRAM AUTO-POLLING LOGIC (CUSTOMER)
// ==========================================
let pollInterval;
// DIKEMASKINI: Bawa masuk data penuh bookingData supaya boleh generate PDF
window.startTelegramPolling = function(docId, bookingData) {
    const trackingId = bookingData.trackingId;
            ...
            if(tgData.ok && tgData.result) {
                for(let update of tgData.result) {
                    if(update.message && update.message.text === `/start ${trackingId}`) {
                        ...
                        // SUNTIKAN 3: Hantar PDF ke Telegram Customer sebagai salinan simpanan dorang
                        const customerPdfBlob = window.generateInvoice(bookingData, true);
                        sendTelegramDocument(chatId, customerPdfBlob, `INVOICE_${trackingId}.pdf`, "Ini adalah salinan Invoice rasmi anda untuk rujukan masa hadapan.");
...
document.getElementById('track-btn')?.addEventListener('click', () => {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        const data = snapshot.docs[0].data(); // Data penuh booking
        ...
        if (!data.telegramId || data.telegramId === "") {
            ...
            // Hantar data penuh untuk proses generate PDF nanti
            newTgBtn.addEventListener('click', () => window.startTelegramPolling(docId, data));
        } else {
...
// ==========================================
// PANDUAN LINK INDIVIDU MULTI-ADMIN TELEGRAM
// ==========================================
window.linkAdminTelegram = function() {
    const btn = document.getElementById('admin-tele-btn');
    if (!btn) {
        console.error("Butang 'admin-tele-btn' tiada dalam fail HTML.");
        return;
    }
    const linkToken = `ADMIN-${Math.floor(1000 + Math.random() * 9000)}`;
    
    btn.innerHTML = `⏳ Waiting...`; 
    btn.style.background = "#555"; 
    btn.disabled = true;
    
    window.open(`https://t.me/${TELEGRAM_BOT_USERNAME}?start=${linkToken}`, '_blank');
    
    let adminPoll = setInterval(async () => {
        try {
            const res = await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/getUpdates`);
            const tgData = await res.json();
            ...
            if(tgData.ok && tgData.result) {
                for(let update of tgData.result) {
                    if(update.message && update.message.text === `/start ${linkToken}`) {
                        const chatId = update.message.chat.id.toString();
                        
                        await setDoc(doc(db, "admin_telegrams", chatId), { 
                            chatId: chatId, 
                            addedAt: new Date() 
                        });
                        
                        sendTelegramMessage(chatId, `🚨 <b>BERJAYA LINK!</b> \nAnda kini didaftarkan sebagai Admin. Semua notifikasi tempahan WAN TECH SOFTWARE akan dihantar ke sini.`);
    ...
    setTimeout(() => {
        ...
        if(btn.innerHTML.includes("Waiting")) {
            btn.innerHTML = `<span style="font-size: 1.1rem; margin-right: 5px;">✈️</span> Retry Link`;
